...
// ── Official Doction Icon — orange D-block (matches app icon exactly) ─────────
...
export default function DoctionLanding() {
  ...
  return (
    <>
      {/* All existing CSS from DoctionLanding — unchanged */}
      <style>{`
        /* ── TOKENS ── */
        :root {
          --bw-bg: #ffffff;
        ...
        .lp-nav-links a { font-size: .84rem; font-weight: 500; color: var(--bw-muted); text-decoration: none; transition: color .15s; letter-spacing: -.01em; }
        ...
        .lp-drawer a, .lp-drawer-link {
          ...
          background: none; border: none; text-align: left; cursor: pointer; transition: background .12s;
        }
        ...
        /* Modal */
        .lp-modal-bg { position: fixed; inset: 0; z-index: 9000; background: rgba(0,0,0,.4); backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; opacity: 0; pointer-events: none; transition: opacity .22s; padding: 16px; }
        .lp-modal-bg.open { opacity: 1; pointer-events: auto; }
        .lp-modal { background: #fff; border-radius: 22px; padding: 36px 32px 32px; width: 100%; max-width: 420px; position: relative; box-shadow: 0 8px 0 rgba(0,0,0,.06), 0 32px 80px rgba(0,0,0,.22); animation: modalIn .26s cubic-bezier(.34,1.56,.64,1) both; }
        @keyframes modalIn { from { opacity: 0; transform: scale(.9) translateY(16px); } to { opacity: 1; transform: none; } }
        .lp-modal-close { position: absolute; top: 16px; right: 16px; width: 30px; height: 30px; border-radius: 50%; border: 1px solid var(--bw-border); background: var(--bw-bg2); cursor: pointer; display: flex; align-items: center; justify-content: center; color: var(--bw-muted); transition: background .15s, color .15s; }
        ...
        .lp-field-input { width: 100%; padding: 11px 14px; border: 1.5px solid var(--bw-border); border-radius: 10px; font-size: .88rem; color: var(--bw-ink); background: #fff; outline: none; transition: border-color .15s, box-shadow .15s; margin-bottom: 14px; font-family: inherit; }
        ...
        @media (max-width: 480px) {
          ...
          .lp-stats-inner { grid-template-columns: repeat(2, 1fr); }
          .lp-modal { padding: 28px 20px 24px; }
        }
      `}</style>
      ...
      <div className="lp-body">
        ...
        <div className={`lp-drawer${drawerOpen ? " open" : ""}`}>
          ...
          <a href="#testimonials" onClick={closeDrawer}>Testemunhos</a>
